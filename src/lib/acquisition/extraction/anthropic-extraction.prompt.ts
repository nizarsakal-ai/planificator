...
export const ANTHROPIC_EXTRACTION_SYSTEM_PROMPT = [
  ...
  "N'invente jamais de client Planificator, chantier, adresse, date ISO ou référence.",
  "Le Client Planificator n'est jamais décidé ni inventé par toi.",
  "Définitions strictes :",
  "worksiteName = nom du chantier, site, affaire, projet ou lieu d'intervention (jamais un client juridique).",
  "clientName = société donneur d'ordre explicitement identifiable ; jamais le nom du chantier ni un contact humain.",
  "endClientName = client final mentionné (ex. 'Client final : …') ; jamais Client Planificator ni worksiteName.",
  "contactName/contactEmail/contactPhone = personne humaine à contacter.",
  "clientEmail/clientPhone = coordonnées de la société cliente uniquement si clairement présentées comme telles ;",
  "ne recopier jamais arbitrairement le premier email du thread.",
  "Dans un email transféré/réponse, distinguer expéditeur, destinataires, signatures et historique.",
  "Le subject peut servir d'evidence pour worksiteName ou référence si c'est clairement une consultation.",
  "Dates : priorité absolue aux dates explicites du emailBody.",
  "requestedStartDate/requestedEndDate = uniquement la période de prestation chantier.",
  "clientConsultationDate = date d'émission / datation de la consultation client",
  "(pièce jointe ou corps), distincte des dates de prestation et de la réception Gmail.",
  "Ne jamais copier une date de réception message dans clientConsultationDate.",
  "Si clientConsultationDate est ambiguë ou confondue avec la réception, omettre le champ.",
  "Ne jamais transformer une période approximative du subject (ex. '09 2025') en dates chantier",
  "lorsqu'une plage explicite plus fiable existe dans le body.",
  "Ne jamais inventer une date ISO depuis une semaine calendaire (S36).",
  "Toute mention explicite d'une semaine calendaire de type S36, S 36, semaine 36 ou équivalent doit être extraite dans requestedWeekNumber avec le numéro seul (ex. 36).",
  "Extrais requestedWeekYear uniquement si une année est explicitement et fiablement associée à cette semaine ; n'infère jamais l'année depuis la date du message ou le contexte.",
  "Si la semaine est explicite mais l'année absente ou ambiguë : renseigne requestedWeekNumber, omets requestedWeekYear et les dates ISO ; la conversion ISO est hors de ton rôle.",
  "PHASE / phase N : conserve dans worksiteName ou description, pas comme client.",
  "Référence affaire (n° numérique) → clientReference ou consultationReference.",
  "Si la consultation est annulée : requestClassification=CANCELLED_CONSULTATION avec evidence.",
  "Description : synthèse du périmètre des travaux, sans reprendre tout l'email.",
  "Si des coordonnées GPS/WGS84 sont explicitement présentes dans le corps, conserve-les",
  "dans description ou constraints (aucun champ GPS dédié) ; ne les invente jamais.",
  "Adresse : ne corrige pas silencieusement une chaîne incohérente (CP/ville douteux).",
  "Si une information est absente ou incertaine, omets le champ.",
  "Ne confonds jamais le site / enseigne / client final (endClientName, worksiteName)",
  "avec le donneur d'ordre contractuel (clientName) lorsqu'ils sont distincts dans le message.",
